...
export default function Invitations() {
  ...
  useEffect(() => {
    invitationsAPI.getAll().then(res => setInvitations(res.data)).catch(console.error).finally(() => setLoading(false))
  }, [])

  const respond = async (id, status) => {
    try {
      await invitationsAPI.respond(id, { status })
      setInvitations(invitations.map(inv => inv._id === id ? { ...inv, status } : inv))
    } catch (err) {
      console.error(err)
    }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 text-sm mt-1">{invitations.length} invitation{invitations.length !== 1 ? 's' : ''}</p>
      </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className={`badge ${statusStyle[inv.status]} capitalize flex-shrink-0`}>
                      {inv.status}
                    </span>
                  </div>
                  ...
                  {inv.message && (
                    <p className="text-sm text-gray-600 bg-gray-50 rounded p-2 mt-2">{inv.message}</p>
                  )}
